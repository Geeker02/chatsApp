...
const HomePage = () => {
  ...
  const getOtherUser = (users) => {
    ...
    return users[0]?.id === auth.reqUser?.id ? users[1] : users[0];
  };
              ...
              {chat.chats.length > 0 && !querys && chat.chats?.map((item) => (
                <div onClick={() => handleCurrentChat(item)}>
                  <hr /> {item.group ? (
                    ...
                    <ChatCard
                      isChat={true}
                      name={getOtherUser(item.users)?.full_name || "Unknown User"}
                      userImg={
                        getOtherUser(item.users)?.profile_picture ||
                ...
                <img className="w-10 h-10 rounded-full"
                  src={
                    currentChat.group
                      ...
                      : (getOtherUser(currentChat.users)?.profile_picture ||
                        "https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_1280.png")
                  }
                />
                <p>
                  {currentChat.group
                    ...
                    : (getOtherUser(currentChat.users)?.full_name || "Unknown User")}
                </p>
              </div>
